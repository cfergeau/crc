...
LDFLAGS := $(VERSION_VARIABLES) -extldflags='-static'
...
.PHONY: install
install: $(SOURCES)
	go install -gcflags="all=-N -l" -ldflags="$(LDFLAGS)" ./cmd/crc

$(BUILD_DIR)/macos-amd64/crc: $(SOURCES)
	GOARCH=amd64 GOOS=darwin go build -gcflags="all=-N -l" -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/macos-amd64/crc ./cmd/crc

$(BUILD_DIR)/linux-amd64/crc: $(SOURCES)
	GOOS=linux GOARCH=amd64 go build -gcflags="all=-N -l" -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/linux-amd64/crc ./cmd/crc
