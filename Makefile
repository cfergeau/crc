...
$(BUILD_DIR)/macos-amd64/crc-macos-installer-amd64-unsigned.tar: packagedir
	tar -cvf $(BUILD_DIR)/macos-amd64/crc--macos-amd64-installer-unsigned.tar ./packaging
...
$(BUILD_DIR)/windows-amd64/crc-windows-amd64-installer-unsigned.zip: $(BUILD_DIR)/windows-amd64/crc-windows-amd64.msi
	rm -f $(HOST_BUILD_DIR)/crc.exe
	rm -f $(HOST_BUILD_DIR)/crc-embedder
	rm -f $(HOST_BUILD_DIR)/split
	pwsh -NoProfile -Command "cd $(HOST_BUILD_DIR); Compress-Archive -LiteralPath $(CABS_MSI) -DestinationPath crc-windows-amd64-installer-unsigned.zip"
