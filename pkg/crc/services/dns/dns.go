...
func CheckCRCPublicDNSReachable(serviceConfig services.ServicePostStartConfig) (string, error) {
	...
	if proxyConfig.IsEnabled() {
		...
		if proxyConfig.ProxyCAFile != "" {
			...
			// We do not really care about transport security for this test, especially
			// as we are connecting through http, all that matters is whether or not we
			// can resolve the hostname, so we can workaround this ambiguity by using an
			// insecure connection
