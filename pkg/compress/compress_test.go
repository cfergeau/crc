...
func testCompress(t *testing.T, baseDir string) {
	...
	fi, err := os.Stat(filepath.Join(destDir, baseDir))
	require.NoError(t, err)
	require.Equal(t, fi.Mode().Perm(), os.FileMode(0700))

	require.NoError(t, checkFileList(filepath.Join(destDir, baseDir), fileList, files))
	require.NoError(t, checkFiles(filepath.Join(destDir, baseDir), files))
}
