...
func vsockListener() (net.Listener, error) {
	...
	logging.Infof(message.Key("listening %s{DefaultURL}", "listening %s"), transport.DefaultURL)
	if err != nil {
		return nil, err
	}
	return ln, nil
...
func httpListener() (net.Listener, error) {
	...
	logging.Infof(message.Key("listening %s{DaemonHTTPSocketPath}", "listening %s"), constants.DaemonHTTPSocketPath)
	if err != nil {
		return nil, err
	}
	return ln, nil
